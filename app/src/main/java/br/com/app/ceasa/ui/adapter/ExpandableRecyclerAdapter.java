...
import br.com.app.ceasa.ui.view.ChildViewHolder;
import br.com.app.ceasa.ui.view.ParentViewHolder;
...
public abstract class ExpandableRecyclerAdapter<PVH extends ParentViewHolder, CVH extends ChildViewHolder>
    ...
    private List<? extends ParentListItemAdapter> mParentItemList;
     ...
     *
     * @param parentItemList List of all {@link ParentListItemAdapter} objects to be
     *                       displayed in the RecyclerView that this
     *                       adapter is linked to
     */
    public ExpandableRecyclerAdapter(List<? extends ParentListItemAdapter> parentItemList) {
        super();
        mParentItemList = parentItemList;
        mItemList = ExpandableRecyclerHelperAdapter.generateParentChildItemList(parentItemList);
     ...
     * that determines if the list item is a parent or a child and calls through
     * to the appropriate implementation of either {@link #onBindParentViewHolder(ParentViewHolder, int,
     * ParentListItemAdapter)}
     ...
     *                               not of type {@link ParentListItemAdapter}
     */
    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        Object listItem = getListItem(position);
        if (listItem instanceof ParentWrapperAdapter) {
            ...
            ParentWrapperAdapter parentWrapperAdapter = (ParentWrapperAdapter) listItem;
            parentViewHolder.setExpanded(parentWrapperAdapter.isExpanded());
            onBindParentViewHolder(parentViewHolder, position, parentWrapperAdapter.getParentListItem());
     ...
     *                        is being created
     * @return A {@code PVH} corresponding to the {@link ParentListItemAdapter} with
     ...
     * @param position         The index in the list at which to bind
     * @param parentListItemAdapter   The {@link ParentListItemAdapter} which holds the data to
     *                         be bound to the {@code PVH}
     */
    public abstract void onBindParentViewHolder(PVH parentViewHolder, int position, ParentListItemAdapter parentListItemAdapter);
     ...
     * @return {@value #TYPE_PARENT} for {@link ParentListItemAdapter} and {@value #TYPE_CHILD}
    ...
    public List<? extends ParentListItemAdapter> getParentItemList() {
        return mParentItemList;
    }

    /**
     * <p>
     * Called when a {@link ParentListItemAdapter} is triggered to expand.
     *
     * @param position The index of the item in the list being expanded
     */
    @Override
    public void onParentListItemExpanded(int position) {
        Object listItem = getListItem(position);
        if (listItem instanceof ParentWrapperAdapter) {
            expandParentListItem((ParentWrapperAdapter) listItem, position, true);
        }
    }

    /**
     * <p>
     * Called when a {@link ParentListItemAdapter} is triggered to collapse.
     *
     * @param position The index of the item in the list being collapsed
     */
    @Override
    public void onParentListItemCollapsed(int position) {
        Object listItem = getListItem(position);
        if (listItem instanceof ParentWrapperAdapter) {
            collapseParentListItem((ParentWrapperAdapter) listItem, position, true);
        }
    }
    ...
    public void expandParent(int parentIndex) {
        ...
        if (listItem instanceof ParentWrapperAdapter) {
            parentWrapperAdapter = (ParentWrapperAdapter) listItem;
        } else {
            return;
        }

        expandViews(parentWrapperAdapter, parentWrapperIndex);
    }

    /**
     * Expands the parent associated with a specified {@link ParentListItemAdapter} in
     * the list of parents.
     *
     * @param parentListItemAdapter The {@code ParentListItem} of the parent to expand
     */
    public void expandParent(ParentListItemAdapter parentListItemAdapter) {
        ParentWrapperAdapter parentWrapperAdapter = getParentWrapper(parentListItemAdapter);
        int parentWrapperIndex = mItemList.indexOf(parentWrapperAdapter);
    ...
    public void expandAllParents() {
        for (ParentListItemAdapter parentListItemAdapter : mParentItemList) {
            expandParent(parentListItemAdapter);
        }
    }
    ...
    public void collapseParent(int parentIndex) {
        ...
        collapseViews(parentWrapperAdapter, parentWrapperIndex);
    }

    /**
     * Collapses the parent associated with a specified {@link ParentListItemAdapter} in
     * the list of parents.
     *
     * @param parentListItemAdapter The {@code ParentListItem} of the parent to collapse
     */
    public void collapseParent(ParentListItemAdapter parentListItemAdapter) {
        ParentWrapperAdapter parentWrapperAdapter = getParentWrapper(parentListItemAdapter);
        int parentWrapperIndex = mItemList.indexOf(parentWrapperAdapter);
        if (parentWrapperIndex == -1) {
            return;
    ...
    public void collapseAllParents() {
        for (ParentListItemAdapter parentListItemAdapter : mParentItemList) {
            collapseParent(parentListItemAdapter);
        }
    }
    ...
    public void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        HashMap<Integer, Boolean> expandedStateMap = (HashMap<Integer, Boolean>) savedInstanceState
                .getSerializable(EXPANDED_STATE_MAP);
        ...
        for (int i = 0; i < parentListItemCount; i++) {
            parentListItemAdapter = mParentItemList.get(i);
     ...
     * @param parentIndex The index of the parent to expand
     */
    private void expandViews(ParentWrapperAdapter parentWrapperAdapter, int parentIndex) {
        PVH viewHolder;
        for (RecyclerView recyclerView : mAttachedRecyclerViewPool) {
            ...
            expandParentListItem(parentWrapperAdapter, parentIndex, false);
        }
    }
     ...
     * @param parentIndex The index of the parent to collapse
     */
    private void collapseViews(ParentWrapperAdapter parentWrapperAdapter, int parentIndex) {
        PVH viewHolder;
        for (RecyclerView recyclerView : mAttachedRecyclerViewPool) {
            viewHolder = (PVH) recyclerView.findViewHolderForAdapterPosition(parentIndex);
            if (viewHolder != null && viewHolder.isExpanded()) {
            ...
            collapseParentListItem(parentWrapperAdapter, parentIndex, false);
        }
    }
     ...
     * total list of items.
     *
     * @param parentWrapperAdapter                     The ParentWrapper of the parent to expand
     * @param parentIndex                       The index of the parent to expand
     * @param expansionTriggeredByListItemClick true if expansion was triggered
     *                                          by a click event, false otherwise.
     */
    private void expandParentListItem(ParentWrapperAdapter parentWrapperAdapter, int parentIndex,
                                      boolean expansionTriggeredByListItemClick) {
        if (!parentWrapperAdapter.isExpanded()) {
            parentWrapperAdapter.setExpanded(true);

            List<?> childItemList = parentWrapperAdapter.getChildItemList();
     ...
     * total list of items.
     *
     * @param parentWrapperAdapter                    The ParentWrapper of the parent to collapse
     * @param parentIndex                      The index of the parent to collapse
     * @param collapseTriggeredByListItemClick true if expansion was triggered
     *                                         by a click event, false otherwise.
     */
    private void collapseParentListItem(ParentWrapperAdapter parentWrapperAdapter, int parentIndex,
                                        boolean collapseTriggeredByListItemClick) {
        if (parentWrapperAdapter.isExpanded()) {
            parentWrapperAdapter.setExpanded(false);
    ...
    public void notifyParentItemInserted(int parentPosition) {
        ParentListItemAdapter parentListItemAdapter = mParentItemList.get(parentPosition);
        ...
        int sizeChanged = addParentWrapper(wrapperIndex, parentListItemAdapter);
    ...
    public void notifyParentItemRangeInserted(int parentPositionStart, int itemCount) {
        ...
        for (int i = parentPositionStart; i < parentPositionEnd; i++) {
            ParentListItemAdapter parentListItemAdapter = mParentItemList.get(i);
            changed = addParentWrapper(wrapperIndex, parentListItemAdapter);
    ...
    private int addParentWrapper(int wrapperIndex, ParentListItemAdapter parentListItemAdapter) {
        int sizeChanged = 1;
        ParentWrapperAdapter parentWrapperAdapter = new ParentWrapperAdapter(parentListItemAdapter);
        mItemList.add(wrapperIndex, parentWrapperAdapter);
    ...
    private int removeParentWrapper(int parentWrapperIndex) {
        int sizeChanged = 1;
        ParentWrapperAdapter parentWrapperAdapter = (ParentWrapperAdapter) mItemList.remove(parentWrapperIndex);
        if (parentWrapperAdapter.isExpanded()) {
            int childListSize = parentWrapperAdapter.getChildItemList().size();
    ...
    public void notifyParentItemChanged(int parentPosition) {
        ParentListItemAdapter parentListItemAdapter = mParentItemList.get(parentPosition);
        int wrapperIndex = getParentWrapperIndex(parentPosition);
        int sizeChanged = changeParentWrapper(wrapperIndex, parentListItemAdapter);
    ...
    public void notifyParentItemRangeChanged(int parentPositionStart, int itemCount) {
        ...
        for (int j = 0; j < itemCount; j++) {
            parentListItemAdapter = mParentItemList.get(parentPositionStart);
            changed = changeParentWrapper(wrapperIndex, parentListItemAdapter);
            sizeChanged += changed;
            wrapperIndex += changed;
            parentPositionStart++;
        }
    ...
    private int changeParentWrapper(int wrapperIndex, ParentListItemAdapter parentListItemAdapter) {
        ParentWrapperAdapter parentWrapperAdapter = (ParentWrapperAdapter) mItemList.get(wrapperIndex);
        parentWrapperAdapter.setParentListItem(parentListItemAdapter);
        int sizeChanged = 1;
        if (parentWrapperAdapter.isExpanded()) {
            List<?> childItems = parentWrapperAdapter.getChildItemList();
    ...
    public void notifyParentItemMoved(int fromParentPosition, int toParentPosition) {
        ...
        ParentWrapperAdapter fromParentWrapperAdapter = (ParentWrapperAdapter) mItemList.get(fromWrapperIndex);
        ...
        boolean isCollapsed = !fromParentWrapperAdapter.isExpanded();
        boolean isExpandedNoChildren = !isCollapsed && (fromParentWrapperAdapter.getChildItemList().size() == 0);
        if (isCollapsed || isExpandedNoChildren) {
            int toWrapperIndex = getParentWrapperIndex(toParentPosition);
            ParentWrapperAdapter toParentWrapperAdapter = (ParentWrapperAdapter) mItemList.get(toWrapperIndex);
            mItemList.remove(fromWrapperIndex);
            int childOffset = 0;
            if (toParentWrapperAdapter.isExpanded()) {
                childOffset = toParentWrapperAdapter.getChildItemList().size();
            }
            mItemList.add(toWrapperIndex + childOffset, fromParentWrapperAdapter);

            notifyItemMoved(fromWrapperIndex, toWrapperIndex + childOffset);
        } else {
            // Remove the parent and children
            int sizeChanged = 0;
            int childListSize = fromParentWrapperAdapter.getChildItemList().size();
            ...
            mItemList.add(toWrapperIndex + childOffset, fromParentWrapperAdapter);
            List<?> childItemList = fromParentWrapperAdapter.getChildItemList();
    ...
    public void notifyChildItemInserted(int parentPosition, int childPosition) {
        ...
        ParentWrapperAdapter parentWrapperAdapter = (ParentWrapperAdapter) mItemList.get(parentWrapperIndex);

        if (parentWrapperAdapter.isExpanded()) {
            ParentListItemAdapter parentListItemAdapter = mParentItemList.get(parentPosition);
            Object child = parentListItemAdapter.getChildItemList().get(childPosition);
    ...
    public void notifyChildItemRangeInserted(int parentPosition, int childPositionStart, int itemCount) {
        ...
        if (parentWrapperAdapter.isExpanded()) {
            ParentListItemAdapter parentListItemAdapter = mParentItemList.get(parentPosition);
            List<?> childList = parentListItemAdapter.getChildItemList();
            Object child;
            for (int i = 0; i < itemCount; i++) {
    ...
    public void notifyChildItemChanged(int parentPosition, int childPosition) {
        ...
        if (parentWrapperAdapter.isExpanded()) {
            ...
            Object child = parentWrapperAdapter.getChildItemList().get(childPosition);
    ...
    public void notifyChildItemRangeChanged(int parentPosition, int childPositionStart, int itemCount) {
        ...
        if (parentWrapperAdapter.isExpanded()) {
            ...
            for (int i = 0; i < itemCount; i++) {
                Object child = parentWrapperAdapter.getChildItemList().get(childPositionStart + i);
    ...
    private HashMap<Integer, Boolean> generateExpandedStateMap() {
        ...
        for (int i = 0; i < listItemCount; i++) {
            if (mItemList.get(i) != null) {
                ...
                if (listItem instanceof ParentWrapperAdapter) {
                    parentWrapperAdapter = (ParentWrapperAdapter) listItem;
                    parentListItemHashMap.put(i - childCount, parentWrapperAdapter.isExpanded());
                } else {
                    childCount++;
                }
            }
    ...
    private int getParentWrapperIndex(int parentIndex) {
        ...
        int listItemCount = mItemList.size();
        for (int i = 0; i < listItemCount; i++) {
            if (mItemList.get(i) instanceof ParentWrapperAdapter) {
                parentCount++;
     ...
     * parents.
     *
     * @param parentListItemAdapter A ParentListItem in the list of parents
     * @return If the parent exists on the list, returns its ParentWrapper.
     * Otherwise, returns null.
     */
    private ParentWrapperAdapter getParentWrapper(ParentListItemAdapter parentListItemAdapter) {
        int listItemCount = mItemList.size();
        for (int i = 0; i < listItemCount; i++) {
            Object listItem = mItemList.get(i);
            if (listItem instanceof ParentWrapperAdapter) {
                if (((ParentWrapperAdapter) listItem).getParentListItem().equals(parentListItemAdapter)) {
                    return (ParentWrapperAdapter) listItem;
                }
            }
        }
